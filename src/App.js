...
import { setGame } from './reducers/gameReducer';
import HelpWidget from './components/HelpWidget';

const getGame = (game) => {
	const GameComp = lazy(() =>
		import(`./componentBundles/games/${game}/${game}`)
	);
	return <GameComp />;
};

function App() {
	const { game, restartButtonStyle } = useSelector((store) => store.game);
	const dispatch = useDispatch();
	const CatalogReturn = () => {
		return (
			<div className='game-out-actions'>
				<div
					className='catalog-return'
					onClick={() => dispatch(setGame('Catalog'))}
				>
					<button>
						<svg
						...
						>
							<defs>
								<clipPath id='id1'>
									<path
										d='M 6.136719 0 L 67 0 L 67 73 L 6.136719 73 Z M 6.136719 0 '
										clipRule='nonzero'
									/>
								</clipPath>
							</defs>
				...
				<div className='restart-game'>
					<button style={restartButtonStyle}>
						<svg
							xmlns='http://www.w3.org/2000/svg'
							xmlnsXlink='http://www.w3.org/1999/xlink'
							width='25'
	...
	return (
		<div className='App'>
			...
			{game !== 'Catalog' ? <>{CatalogReturn()}</> : null}
			{game === 'Catalog' ? (
...
// Problems I encountered during the course of this project
// rendering game based on state, without hardcoded conditions since the number of games could be huge.
// Solution => Lazy loading, using React.lazy to load the game component dynamically.
// Using a single state to store the restart game function solution => Redux
